...
export default function BriefPage() {
  ...
  const openModal = (m: ModalMode) => {
    if (m.type === 'raise-flag') {
      ...
      const existing = m.log.mom
      setMomForm({
        title: existing?.title ?? m.log.title,
        date: existing
          ? format(new Date(existing.date), 'yyyy-MM-dd')
          : format(new Date(m.log.postedAt), 'yyyy-MM-dd'),
  ...
  const submitAddFlag = async () => {
    ...
    const res = await fetch('/api/logs', {
      ...
      body: JSON.stringify({
        ...
        deadline: flagForm.deadline || null,
      }),
    })
    if (res.ok) { closeModal(); await load() }
    setSaving(false)
  ...
  const submitMOM = async () => {
    ...
    const res = await fetch('/api/moms', {
      ...
      body: JSON.stringify({
        logId: modal.log.id,
        title: momForm.title.trim(),
        date: momForm.date,
        stakeholders: momForm.stakeholders.trim(),
        notes: momForm.notes,
  ...
  if (!brief) {
    return (
      <div className="neu p-10 text-center max-w-sm">
        <p style={{ color: 'var(--text-2)' }}>Brief not found.</p>
      </div>
    )
  }
  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="flex items-center gap-2 mb-6 flex-wrap">
        <Link href="/houses" className="text-sm" style={{ color: 'var(--accent)', textDecoration: 'none' }}>
          Houses
        </Link>
        <span style={{ color: 'var(--text-3)' }}>›</span>
        <Link href={`/houses/${houseId}`} className="text-sm" style={{ color: 'var(--accent)', textDecoration: 'none' }}>
          {brief.house.name}
        </Link>
        <span style={{ color: 'var(--text-3)' }}>›</span>
      ...
      <div className="flex items-start justify-between mb-8 gap-4">
        <div>
          ...
          {brief.description && (
            <p className="text-sm mt-1" style={{ color: 'var(--text-2)' }}>{brief.description}</p>
          )}
        </div>
        <div className="flex gap-2.5 shrink-0">
          <button className="neu-btn neu-btn-sm" style={{ color: 'var(--text-2)' }} onClick={() => openModal({ type: 'add-log' })}>
            + Log
          </button>
          <button className="neu-btn neu-btn-sm" style={{ color: 'var(--accent)' }} onClick={() => openModal({ type: 'add-flag' })}>
            🚩 Flag
          </button>
        </div>
      </div>
      ...
      {logs.length === 0 ? (
        <div className="neu p-14 text-center">
          ...
          <p className="text-sm" style={{ color: 'var(--text-2)' }}>
            Add a log to record activity, or raise a flag directly for a task.
          </p>
      ...
      {/* Stats */}
      {logs.length > 0 && (
        <div className="flex gap-3 mt-6 text-xs flex-wrap" style={{ color: 'var(--text-3)' }}>
        ...
        <Modal title="Add Log" onClose={closeModal}>
          <div className="flex flex-col gap-4">
            <div>
              ...
              <input
                className="neu-input"
                placeholder="What happened, what was shipped…"
                value={logForm.title}
                autoFocus
      ...
      {modal?.type === 'raise-flag' && (
        <Modal title="Raise as Flag 🚩" onClose={closeModal}>
          <p className="text-sm -mt-2 mb-2" style={{ color: 'var(--text-3)' }}>
            This log will become a Brief Flag. Fill in the details below.
          </p>
        ...
        <Modal title={modal.log.mom ? 'Edit MOM 📋' : 'Create MOM 📋'} onClose={closeModal} width="max-w-2xl">
          <div className="flex flex-col gap-4">
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <input
                  ...
                  className="neu-input"
                  value={momForm.date}
                  onChange={e => setMomForm(f => ({ ...f, date: e.target.value }))}
                />
              </div>
            </div>
            ...
            <div>
              ...
              <RichEditor
                value={momForm.notes}
                onChange={v => setMomForm(f => ({ ...f, notes: v }))}
            ...
            <div>
              ...
              <RichEditor
                ...
                onChange={v => setMomForm(f => ({ ...f, decisions: v }))}
                placeholder="What was decided, what changes…"
                minHeight="8rem"
              />
            </div>
...
function LogItem({
  ...
  return (
    <div
      className="neu-md p-4 flex flex-col gap-2.5"
      style={{ opacity: entry.completed ? 0.65 : 1, transition: 'opacity 0.2s' }}
    >
      {/* Top row */}
          ...
          <div className="flex items-center gap-2 flex-wrap mb-0.5">
            ...
            {entry.mom && (
              <button
                ...
                title={momOpen ? 'Collapse MOM' : 'Expand MOM'}
              >
                📋 MOM {momOpen ? '▲' : '▼'}
              </button>
            )}
          ...
          <div className="flex items-center gap-3 flex-wrap">
            ...
            {entry.deadline && (
              <span className="text-xs font-medium" style={{ color: overdue ? 'var(--danger)' : 'var(--text-3)' }}>
                ...
                {!overdue && !entry.completed && (
                  <span className="ml-1 opacity-70">
                    ({formatDistanceToNow(new Date(entry.deadline), { addSuffix: true })})
                  </span>
                )}
              </span>
            )}
      ...
      {entry.isFlag && entry.description && (
        <p
          ...
          style={{
            color: 'var(--text-2)',
            borderLeft: '2px solid var(--accent-bg)',
            paddingLeft: '0.75rem',
            marginLeft: entry.isFlag ? '1.625rem' : '0',
          }}
        >
          {entry.description}
        </p>
      ...
      {/* MOM inline panel */}
      {entry.mom && momOpen && (
        <MOMPanel mom={entry.mom} onEdit={onCreateMOM} onDelete={onDeleteMOM} />
      )}
    </div>
  )
}
...
function ActionsDropdown({
  ...
  return (
    <div ref={ref} className="relative shrink-0">
      <button
        className="neu-btn neu-btn-sm w-7 h-7 p-0 flex items-center justify-center"
        style={{ color: 'var(--text-3)', letterSpacing: '0.05em', fontSize: '0.9rem' }}
        onClick={() => setOpen(v => !v)}
        title="Actions"
      >
        ···
      ...
      {open && (
        <div
          className="neu-md absolute right-0 top-full mt-1.5 z-30 py-1.5 flex flex-col"
        ...
        >
          {!entry.isFlag && (
            <DropItem
              icon="🚩"
              label="Raise as Flag"
              onClick={() => { setOpen(false); onRaiseFlag() }}
            />
          )}
          <DropItem
            icon="📋"
            label={entry.mom ? 'Edit MOM' : 'Create MOM'}
            onClick={() => { setOpen(false); onCreateMOM() }}
          />
...
function DropItem({
  ...
  return (
    <button
      onClick={onClick}
      className="flex items-center gap-2.5 px-3.5 py-2 text-sm text-left w-full transition-colors"
      style={{
        background: 'none',
        border: 'none',
        cursor: 'pointer',
      ...
      onMouseEnter={e => {
        ;(e.currentTarget as HTMLElement).style.color = danger ? 'var(--danger)' : 'var(--text-1)'
      }}
      onMouseLeave={e => {
        ;(e.currentTarget as HTMLElement).style.color = danger ? 'var(--danger)' : 'var(--text-2)'
      }}
    >
      <span style={{ fontSize: '0.9rem', width: '1rem', textAlign: 'center' }}>{icon}</span>
      <span>{label}</span>
    </button>
  )
}
...
function MOMPanel({ mom, onEdit, onDelete }: { mom: MOM; onEdit: () => void; onDelete: () => void }) {
  ...
  return (
    <div
      className="neu-inset-sm mt-1 p-4 flex flex-col gap-4"
    ...
    >
      ...
      <div className="flex items-start justify-between gap-3">
        ...
        <div className="flex gap-1.5 shrink-0">
          <button
            ...
            style={{ color: 'var(--accent)', fontSize: '0.75rem', padding: '0.2rem 0.6rem' }}
            onClick={onEdit}
          >
            Edit
          </button>
          <button
            className="neu-btn neu-btn-sm w-6 h-6 p-0 flex items-center justify-center"
      ...
      {mom.notes && (
        <div>
          ...
          <div
            ...
            dangerouslySetInnerHTML={{ __html: mom.notes }}
          />
        </div>
      )}
      ...
      {mom.decisions && (
        <div>
          ...
          <div
            className="rich-content"
            dangerouslySetInnerHTML={{ __html: mom.decisions }}
          />
        </div>
      )}
    </div>
    ...
    <div className="flex flex-col gap-4">
      ...
      <div>
        ...
        <textarea
          className="neu-input"
          rows={3}
          placeholder="Context, links, criteria…"
      ...
      <div className="flex gap-3 pt-1">
        ...
        <button className="neu-btn" style={{ color: 'var(--text-3)' }} onClick={onCancel}>
          Cancel
        </button>
      </div>
    </div>
